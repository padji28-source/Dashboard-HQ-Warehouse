...
export default function PencocokanData({ spreadsheetId, area }: { spreadsheetId: string; area: string }) {
    ...
    try {
      ...
      combined.forEach(s => {
        if (!uniqueMap.has(s.id)) {
          ...
          // If Firestore record has fireId, merge it so we keep the edit reference
  ...
  const reconciliationList = useMemo(() => {
    ...
    allTransactions.forEach(t => {
      ...
      // Date ranges check for daily vs monthly
    ...
    return Array.from(listMap.values()).map(item => {
      ...
      // Compute selisih and status based on physical stock and MTS system stock
  ...
  const handleSaveSessionConfirm = async () => {
    ...
    try {
      ...
      // 1. Save to Cloud FireStore (non-blocking)
  ...
  return (
    <div className="space-y-6">
      ...
      {activeSavedSession && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm animate-fade-in">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-amber-850 mt-0.5">
      ...
      {/* Mode Sub-nav Tabs (Daily vs Monthly) - Only shown if not in archive mode */}
      {!activeSavedSession && (
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-xl px-4 py-3 text-xs sm:text-sm text-blue-800 flex items-center gap-3">
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <table className="w-full text-left text-sm whitespace-nowrap divide-y divide-slate-150">
              {/* Conditional rendering of different tables for Daily vs Monthly */}
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {displayedList.length === 0 ? (
                  ...
                  paginatedReconciliation.map(item => (
                    <tr key={item.key} className="hover:bg-blue-50/20 transition-colors text-slate-700">
                      {/* Area (Only for HQ in Monthly mode) */}
                      ...
                      {currentReconType === 'daily' && (
                        <>
                          {/* Stok Rill kemarin = stokRill - mutasiQty */}
                          <td className="px-5 py-4 text-right font-medium text-slate-700 bg-slate-50/25">
                            {(item.stokRill - item.mutasiQty).toLocaleString()}
                          </td>
                          ...
                          <td className="px-5 py-4 text-center">
                            {item.status === 'SESUAI' && (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-850">
                                <Check className="w-3.5 h-3.5" />
                                Sesuai (Match)
                              </span>
                            )}
                            {item.status === 'SELISIH' && (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-rose-100 text-rose-850 animate-pulse">
                ...
                {!loading && displayedList.length > 0 && (
                  <tr className="bg-slate-100/60 border-t-2 border-slate-300 font-bold text-slate-900">
                    <td 
                      colSpan={currentReconType === 'daily' ? 2 : ((area === 'HQ' || spreadsheetId === 'HQ') ? 3 : 2)} 
                    ...
                    {currentReconType === 'daily' && (
                      <>
                        ...
                        <td className="px-5 py-4 text-right text-slate-600 font-bold text-sm bg-slate-100/10">
                          {(displayedTotals.stokRill - displayedTotals.mutasiQty).toLocaleString()}
                        </td>
      ...
      {/* HISTORI SESI REKONSILIASI TERKUNCI (LOCK & SAVE) SECTION */}
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden p-6 space-y-4 animate-fade-in">
            ...
            {savedSessions.map((sess) => (
              <div 
                ...
                className={cn(
                  ...
                  activeSavedSession?.id === sess.id 
                    ? "border-amber-400 bg-amber-50/20 ring-1 ring-amber-450" 
              ...
              >
                <div>
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-3 gap-1 bg-slate-50/85 rounded-lg p-2 text-center text-[11px] text-slate-505 font-mono">
                    <div>
                      <div className="font-semibold text-[9px] text-slate-400 uppercase">Awal</div>
                      <div className="font-bold text-slate-800">{(sess.grandTotals?.stokRill - sess.grandTotals?.mutasiQty || 0).toLocaleString()}</div>
                    </div>
                    <div>
                ...
                <div className="flex items-center gap-2 mt-4 pt-2 border-t border-slate-100">
                  ...
                  <button
                    ...
                    className="p-1.5 border border-slate-150 rounded-lg hover:bg-rose-50 hover:text-rose-700 text-slate-400 transition-colors focus:outline-none"
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex gap-3 text-slate-600 text-xs sm:text-sm">
        ...
        <div>
          <span className="font-semibold text-slate-805 font-bold">Keterangan Prosedur Rekonsiliasi:</span> Kolom <span className="font-semibold font-bold">Mutasi</span> menghitung transaksi IN/OUT yang dicatat di tanggal/bulan berjalan. Kolom <span className="font-semibold font-bold">Stok Rill (Kemarin atau Awal Bulan)</span> dihitung dari akumulasi transaksi sebelum batas interval. Selisih dihitung terhadap <span className="font-semibold font-bold">Stok Sistem (MTS)</span> dari Google Sheets. Data ini dapat Anda simpan secara permanen ke database lokal & cloud dengan mengklik tombol "Kunci & Simpan Sesi".
        </div>
      </div>
      ...
      {showSaveModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xl w-full max-w-lg space-y-4 text-left my-8">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-650 leading-relaxed font-mono space-y-1">
              ...
              <div>• Grand Total Selisih: <strong className={grandTotals.selisih === 0 ? "text-emerald-750" : "text-rose-750"}>{grandTotals.selisih.toLocaleString()}</strong></div>
            </div>
