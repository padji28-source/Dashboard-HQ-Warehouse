...
export default function MtsData() {
  ...
  const loadData = async () => {
    try {
      ...
      // Find the first row that has substantial data to serve as header
      let headerIndex = 0;
      for (let i = 0; i < Math.min(10, data.length); i++) {
        // Look for typical headers or simply a row with most columns populated
      ...
      // Clean rows to matching columns length, filtering out fully empty rows
  ...
  // Find indices of specific columns to display in order: Locator - Search Key - Name - UOM - START QTY - LAST QTY
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      ...
      // Check if both are numbers for numeric comparison
  ...
  // Effect to reset page on search change
  useEffect(() => {
    setCurrentPage(1);
  }, [search, pageSize]);
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="p-20 flex flex-col items-center justify-center text-slate-400 gap-3">
              ...
              <div className="text-xs text-slate-400 max-w-xs text-center">Megunduh file CSV dan menguraikan data baris dengan PapaParse.</div>
            </div>
          ) : (
            <>
                        ...
                        {displayIndices.map((col, cIdx) => (
                          <td 
                            ...
                            className={cn(
                              ...
                              // Highlight status or actions if detected
